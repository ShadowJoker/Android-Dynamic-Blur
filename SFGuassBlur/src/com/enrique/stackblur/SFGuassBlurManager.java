...
public class SFGuassBlurManager {
    static final int EXECUTOR_THREADS = Runtime.getRuntime().availableProcessors();
    static final ExecutorService EXECUTOR = Executors.newFixedThreadPool(EXECUTOR_THREADS);
    static final int scaleRatio = 1;// 缩放比例，例如8代表1/8

    private static volatile boolean hasRS = true;
    private volatile BlurProcess blurProcess = null;
     ...
     */
    public SFGuassBlurManager(Bitmap image) {
        _image = image;
    ...
    public Bitmap processNatively(int radius) {
        NDKBlurProcess blur = new NDKBlurProcess();
    ...
    public Bitmap processRenderScript(Context context, float radius) {
        ...
        // and fall back to NDKBlurProcess
        if (blurProcess == null) {
            if (hasRS) {
                try {
                    ...
                    if (BuildConfig.DEBUG) {
                        Log.i("SFGuassBlurManager", "Falling back to Native Blur", e);
                    }
                    blurProcess = new NDKBlurProcess();
                    hasRS = false;
                }
            } else {
                blurProcess = new NDKBlurProcess();
